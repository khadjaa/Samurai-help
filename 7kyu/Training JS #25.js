...
The parameter compareFunction is optional. Specifies a function
that defines the sort order. If omitted, the array is sorted
...
This is an example of an array of strings, if arr is a number
array and the parameter is omitted, sort() will first convert
 the number into a string, and then sort the array. Example:
...
//output:
[ 1, 11, 2, 21, 3, 31 ]
This result may not be what we want. If you want to sort the
numbers according to the size of their values, you must
 use a parameter, the parameter is a function. A classic example:
...
Usually this is the result we want. (a,b)=>a-b is arrow function.
 The formal format is: function(a,b){return a-b}. If you forget
  how to use the arrow function, please refer to:#22:
  Unlock new skills--Arrow function,spread operator and deconstruction

The principle of sorting is:
(Text is too long, if you are not interested in reading please skip)
...
If we need to sort by two or more conditions, we need to spend more
energy to write a proper comparison function. The following example
has two sorting conditions, first according to the classification
of odd and even order, and then follow the numbers in ascending order.
...
according to the number values(in descending order) for example:
