...
Each answer should be given as ["M: f", "B: b", "C: c"] and all
the answers as [ ["M: f", "B: b", "C: c"], ... ]. "M" stands for money, "B" for boats, "C" for cars.
...
function howMuch(m, n) {
    ...
    for (let f = Math.min(m, n); f <= Math.max(m, n); f++) {
        const boats = (f - 2) / 7;
        const cars = (f - 1) / 9;

        if (Number.isInteger(boats) && Number.isInteger(cars)) {
            result.push([`M: ${f}`, `B: ${boats}`, `C: ${cars}`]);
        }
    }
